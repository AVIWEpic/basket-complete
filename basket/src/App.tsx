...
function App() {
  ...
  return <div className="App">{pageBody()}</div>;

  function pageBody() {
    if (error)
      return <Message notificationText={error} notificationType="error" />;
    if (basket)
      return (
        <BasketView
          basket={basket}
          updateAndViewBasket={updateAndViewBasket}
    ...
    if (products)
      return (
        <Products
          ...
          updateBasketItems={setBasketItems}
